...
3. 새 화면·컴포넌트의 스타일(JSX/StyleSheet)을 작성하기 직전에 `DESIGN.md`가 있으면 읽고 그 안의 토큰(colors/typography/spacing/radius)과 Named Rules(예: One Accent Rule, Tinted Shadow Rule)를 따른다. `src/theme.ts`·`src/lib/themePresets.ts`에 이미 토큰화된 팔레트는 그대로 따른다. DESIGN.md가 다루지 않는 결정(새로운 종류의 컴포넌트, 모션 등)만 `frontend-design` 스킬로 보완한다
